...
export default function EcoVoiceSummary() {
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3">
        ...
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.35em] text-emerald-300">{t('voiceSummary.title')}</p>
          <h2 className="mt-1 text-2xl font-semibold text-white">{t('voiceSummary.heading')}</h2>
        </div>
      </div>

      <div className="mt-6 flex flex-wrap gap-3">
        {[
          { label: t('voiceSummary.readReport'), text: t('voiceSummary.reportReady') },
          { label: t('voiceSummary.explainInTamil'), text: t('voiceSummary.tamilReport'), lang: 'ta' },
          { label: t('voiceSummary.explainInEnglish'), text: t('voiceSummary.lowerImpact'), lang: 'en' },
          { label: t('voiceSummary.askEcoVoice'), text: t('voiceSummary.askEcoVoicePrompt') }
        ].map(({ label, text, lang }) => (
          <button
            key={label}
            type="button"
